...
\scriptsize{
\begin{itemize}
\item In this class we will learn how to draw estimates from a posterior distribution.

\item In the theoretical world (when the posterior has a closed mathematical expressions), this implies calculating complicated integrals to cancel out (or average) different variables.

\item In the practical world, same results can be obtained  using \textbf{samples} from the posterior.
...
\item Once your model produces a posterior distribution, the model's work is done.
...
\item We’ll see how to approach these questions using samples from the posterior. 
\end{itemize}
