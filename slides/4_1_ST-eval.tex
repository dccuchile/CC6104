...
\begin{equation}
 H(X) = - \mathbb{E}(\log f(x_i)) = -\sum_{i=1}^{m} 
\end{equation}

\item If $X$ is continuos with density function $f$, the entropy $H(X)$ takes the following form:

\begin{equation}
 H(X) = - \mathbb{E}(\log f(x)) = -\int f(x)\log f(x)dx
\end{equation}

\item In plainer words: ``The uncertainty contained in a probability distribution is the average log-probability of an event''.
...
\item To compute the information entropy for the weather, suppose the true probabilities of rain ($X=1$) and shine ($X=2$) are $f(1) = 0.3$ and $f(2) = 0.7$, respectively. 

\item Then:

\begin{displaymath}
 H(X) = - (f(1)\log f(1)+\left(f(2)\log f(2)\right) \approx 0.61
\end{displaymath}
...
\item Now the entropy is about 0.06. 
...
\item This is often known as Kullback-Leibler divergence or simply K-L divergence, named after the people who introduced it for this purpose.
