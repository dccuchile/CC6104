...
\item And is actually an approximation of K-L divergence. 
...
\begin{itemize}
\item We can calculate AIC for the model reg.ev.1 as follows:
...
\item AIC can be computed directly with the R function AIC:
...
\item In AIC, the lower the better, so the criterion is still preferring overfitted models.
...
\item All information criteria aim at this same target, but are derived under more and less general assumptions. AIC is just the oldest and most restrictive.
\item AIC is usually used for frequentist models fitted with maximum likilihood.
...
\item What we just did is using information criteria for model comparison.
...
\begin{itemize}
\item With definition of DIC in hand, let's review one more simulation exercise.
...
\item Regularization, as long as it's not too strong, reduces  overfitting for any particular model. \item Information criteria instead help us measure overfitting across models fit to the same data. \item These are complementary functions. 
...
\item Theree common tools were introduced to address over-fitting: regularizing priors, cross-validation, and information criteria.
\item Regularizing priors reduce overfitting during estimation, and both cross-validation and information criteria help estimate the degree of overfitting. 
