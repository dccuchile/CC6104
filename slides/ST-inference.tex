...
\begin{itemize}
 ...
 \item For example, we can asssume that each data point is generated by $N(\mu,\sigma^2)$, then we will compute the joint PDF of our data and find the parameter values for $\mu$ and $\sigma$ that maximize that joint density.
...
\scriptsize{
\begin{itemize}
 \item Let $X_1,\dots,X_n$ be IID with PDF $f(x;\theta)$.
 \item Since we are assuming that our data samples are independent random variables, the joint density (PDF) would be the product of each PDF: 
 \begin{displaymath}
\mathcal{L}_{n}(\theta)=\prod_{i=1}^nf(X_i;\theta)  
 \end{displaymath}
\item We refer to this joint density as the \textbf{likelihood function}.
\item The likelihood function is just the joint density of the data, except that we treat it is a function of the parameter $\theta$.
...
\item The maximum likelihood estimator MLE, denoted by $\hat{\theta}_n$, is the value of e that maximizes $\mathcal{L}_{n}(\theta)$.
...
\item Take the derivative of $l_n(\theta)$, set it equal to 0 to find that the MLE is $\hat{p}_n =S/n$, which is the same value as the sample provision shown earlier.
