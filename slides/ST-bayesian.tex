...
$\theta$ & $P(\theta)$ & Ways to Produce Data & $P(d|\theta)$ & $P(\theta|d) = P(d|\theta)*P(\theta) / P(d)$  \\ \hline
...
\item where P(d)= 1/5 * 0/64 + 1/5*3/64 + 1/5*8/64+1/5*9/64+1/5*0/64 = 0.0625 
...
\scriptsize{
\begin{itemize}
\item Let's use the factory counts information (blue marbles are are) now in our prior assumptions of $P(\theta)$.  \\ 
\item This can be done by normalizing the factory counts.  

\item Notice that this new prior assumption doesn't affect the ways each conjecture can generate the data and $P(d|\theta)$ remains unchanged.
 
 \vspace{0.3cm}
  \begin{tabular}{c|c|c|c|c} \hline
$\theta$ & Factory count & $P(\theta)$ &  $P(d|\theta)$ & $P(\theta|d) = P(d|\theta)*P(\theta) / P(d)$  \\ \hline
...
\item where P(d)= 0/6 * 0/64 + 3/6*3/64 + 2/6*8/64+1/6*9/64+0/6*0/64 =  0.08854167

\item Two different prior assumptions led us to different values of $\mathbb{P}(d|\theta)$.
