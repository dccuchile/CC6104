...
\begin{itemize}
\item The binomial distribution is the de facto distribution for this kind of ``coin tossing'' problem:
\begin{displaymath}
...
\item We will assume that all possible values are equally likely, which implies that p follows a \textbf{continous Uniform distribution} between 0 and 1.
\begin{displaymath}
 \mathbb{P}(p) = \text{Uniform}(0,1) = 1/(b-a) \text{ where b=1, and a=0} = 1
\end{displaymath}

\item This flat prior assumes that $p=0$, $p=0.5$ and $p=1$ are all equally plausible.
\item This is not the best prior information we can declare, considering that we already know that the earth cannot be completely covered by the earth ($p=0$) or by water ($p=1$).
...
\item It can be obtained using the Bayes formula:

\begin{displaymath}
 \mathbb{P}(p|W,L) = \frac{\mathbb{P}(W,L|p)* \mathbb{P}(p)}{\mathbb{P}(W,L)}
\end{displaymath}
...
\begin{itemize}
\item The posterior of our globe model with binomial likelihood and uniform prior has a closed and follows a Beta distribution.
\begin{displaymath}
...
\begin{displaymath}
\text{Beta}(\alpha,\beta)=\frac{x^{\alpha-1}(1-x)^{\beta-1}}{\frac{\Gamma(\alpha)\Gamma(\beta)}{\Gamma(\alpha + \beta)}}
\end{displaymath}


\item Our posterior $\mathbb{P}(p|W,L)$ is proportional to $\mathbb{P}(W,L|p)*\mathbb{P}(p)$.
\item Since $\mathbb{P}(p)=1$ we have that
\begin{displaymath}
\mathbb{P}(p|W,L) \sim  p^W(1-p)^L*1
\end{displaymath}
...
% Binomial + uniform = Beta https://stats.stackexchange.com/questions/185221/binomial-uniform-prior-bayesian-statistics
