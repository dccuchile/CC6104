...
\item The parameters are a mean vector $\vec{\mu} \in \mathcal{R}^d$ containing to the mean of each dimension.

\item Aand a covariance matrix $\Sigma \in R^{d\times d}$, where $\Sigma \geq 0$ is symmetric and positive semi-definite.

\item This matrix contains the variance of each variable in the diagonal and the covariance of variables $X_i$ and $X_j$ in the other cells $\Sigma_{i,j}$  \cite{ng2008generative}:
...
\end{displaymath}
  ...
  \item It is a very convenient distribution for modeling multidimensional random variables.
...
\begin{itemize}
  ...
  \item Here are some examples of what the density of a multivariate Gaussian distribution looks like:
