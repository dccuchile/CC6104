...
\begin{itemize}
 ...
 \item Here ``male'' is a binary or dummy variable (takes the value 1 when the person is male and 0 otherwise).
 
 \item We make sure first that the ``male'' is represented as a factor:
...
\scriptsize{ 
\begin{itemize}
\item Here $\beta_0$ is the average height of females and $\beta_0$ is the average height difference of between men and women:
...
\scriptsize{ 
\begin{itemize}
\item The p-value of the statistical test for $\beta_1$ in the regression is the same as what we get from an unpaired two Sample t-tests with equal variance:
...
\item In this model we use the same slope relating height to weight for both groups.
...
\scriptsize{ 
\begin{itemize}
\item If we want to fit a model using lines with separate slopes for each category,  we need to include an interaction.
\item This is equivalent to fitting different lines for each of the two groups but with one single call.
\item In R, interactions are denoted by using the * or : symbol in the model.
\begin{verbatim}
...
\scriptsize{ 
\begin{itemize}
\item Let's compare the coefficients of the model with interactions vs the coefficients of models independently fitted for each category:
\begin{verbatim}
> d.male<-d[d$male==1,]
> d.female<-d[d$male==0,]
...
\scriptsize{ 
\begin{itemize}
\item We can \verb+weight:male1+ from reg6 encodes the difference in slopes between both groups.
