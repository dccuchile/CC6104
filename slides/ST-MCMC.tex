...
\item Rhat is a complicated estimate of the convergence of the Markov chains to the target distribution. It should approach 1.00 from above, when all is well.

\item We can also control the number of samples from the chain by using the iter and warmup parameters. 
\item The defaults are 1000 for iter and warmup is set to iter/2 , which gives you 500 warmup samples and 500 real samples to use for inference.

\item 

\item Here we are sampling 4 different chains distributed over 4 CPU cores.
