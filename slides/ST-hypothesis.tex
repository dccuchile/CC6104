...
\begin{itemize}
\scriptsize{
 \item The null hypothesis always involves some kind of equality ($=$, $\leq$, or $\geq$).}
\end{itemize}
...
\begin{itemize}
 \item Calculate a critical region of values such that if $T$ lies in this region then we will reject $H_0$ 
 \item Compute the probability of the observed value (or more extreme values) of that statistic assuming that the null hypothesis is true.
