...
[1] TRUE
 ...
 ``there is signicant evidence against the null hypothesis at the 5\% level''.
...
\scriptsize{
 ...
 \item In many cases  $\sigma$ is unknown and we must estimate it using the unbiased estimator $s$ that we saw in previous class.
 \item In these cases we can calculate a $T$ statistic $T=\frac{\overline{X_{n}}-\mu_{o}}{\frac{s}{\sqrt{n}}}$
 \begin{verbatim}
> s<-sd(babyboom$wt)
> s
[1] 528.0325
...
\scriptsize{
 ...
 \begin{itemize}
 \item From previous class we know that follows a t-student distribution with $n-1$ degrees of freedom  $T \sim t_{n-1}$.
 \item When we perform the test using t-student distribution we are using a T-test.
 
 \item The p-value can be calculated analougusly to the previous case now using the t-student distribution.
...
[1] 0.0006042622
 ...
 \item This is becasue the t-distribution has wider tails than the Normal distribution.
...
\scriptsize{
 ...
 \begin{itemize}
 \item We can perform t-tests directly in R as follows:
...
\begin{itemize}
 ...
 \begin{itemize}
 \scriptsize{
 \item  In parametric tests, $T$ is a standardized value that we can check in a distribution table (e.g., a Z-score).}
 \end{itemize}
...
\begin{frame}[fragile]{Single-sample Tests}
\scriptsize{
\begin{itemize}
