...
\item We need to keep in mind that  there are only four types of variable relations that combine to form all possible undirected paths in a DAG between any pair of variables X and Y with one inmmediate neightbor Z: 

  \begin{figure}[h!]
	\centering
	\includegraphics[scale=0.6]{pics/collider.png}
	\caption{source: \cite{mcelreath2020statistical}}
	\end{figure} 
...
\item An undirected path from $X$ to $Y$ is d-connecting with respect to the evidence nodes $E$ if \textbf{every} interior node $E_i$ in the path has the property that either:
