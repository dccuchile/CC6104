...
\item Note that we could use the chain rule to represent the joint probability with any order.

\item For example  $f(I,S) = f(S)f(I|S)$
...
\item Her performance on the SAT is determined (stochastically) by her  intelligence. 
