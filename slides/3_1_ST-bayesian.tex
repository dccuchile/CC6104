...
\item For any positive integer $n$, the gamma function satisfies $\Gamma(n) = (n-1)!$

\item Therefore, if $W$ and $L$ are integers, the Beta \emph{pdf} with parameters $(W+1, L+1)$ is
\begin{displaymath}
\mathrm{Beta}(p \mid W+1,L+1)
= \frac{p^{W}(1-p)^{L}}{B(W+1,L+1)}
= \frac{(W+L+1)!}{W!\,L!}\,p^{W}(1-p)^{L}
\end{displaymath}

\item This expression closely resembles the binomial distribution:
\[
\binom{W+L}{W}p^W(1-p)^L
\]
but the Beta density uses $(W+L+1)!$ instead of $(W+L)!$.

\item Interpretation:
\begin{itemize}\scriptsize
\item the Binomial distribution models the number of successes $W$,
\item the Beta distribution models our uncertainty about the probability $p$ of success.
\end{itemize}
...
\item With a uniform prior $f(p)=1$, the posterior is proportional to the likelihood:
\[
f(p \mid W,L)\propto f(W,L\mid p) = p^W(1-p)^L
\]

\item This kernel is exactly the form of a Beta density, so the normalized posterior must be
\[
f(p \mid W,L) = \mathrm{Beta}(p \mid W+1, L+1)
\]
...
\item The denominator in Bayes’ rule is
\[
\int_0^1 f(W,L\mid p)\,dp
= \int_0^1 \frac{(W+L+1)!}{W!\,L!}\,p^W(1-p)^L \, dp
\]
which is equal to 1 because we are integrating a normalized Beta pdf over its entire support.
...
\item Using the fact that the denominator integrates to 1 (because it normalizes a Beta kernel), we obtain:

\[
f(p \mid W,L) = \frac{(W+L+1)!}{W!\,L!}\,p^W(1-p)^L
= \mathrm{Beta}(p \mid W+1 , L+1)
\]

\item Therefore, in our globe–tossing example $(W=6,L=3)$, the posterior can be computed analytically as:

\[
f(p \mid W=6,L=3) = \mathrm{Beta}(p \mid 7,4)
\]

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.4]{pics/beta(7,4).pdf}
\end{figure}
